...
export default function Page() {
  ...
  async function refreshTable() {
    ...
    try {
      ...
      addAlert(
        //@ts-ignore
        e.response?.data ?? e.message ?? "Error refreshing stations",
        "error"
      );
      return false;
    } finally {
  ...
  async function onCreate(name: string, mqttID: string, notes?: string) {
    try {
      ...
      addAlert(
        //@ts-ignore
        e.response?.data ?? e.message ?? "Error creating station",
        "error"
      );
      return false;
    }
  ...
  async function onUpdate(
    ...
    try {
      const res = await axios.patch("/api/db/stations/update-station/" + id, {
        name: name,
        mqtt_id: mqtt_id,
      ...
      addAlert(
        //@ts-ignore
        e.response?.data ?? e.message ?? "Error updating station",
        "error"
      );
      return false;
    }
  ...
  async function deleteStation(id: number) {
    try {
      const res = await axios.delete("/api/db/stations/delete-station/" + id);
      addAlert(res.data, "success");
      await refreshTable();
          ...
          return [
            ...
            <div key={i} className="flex flex-row gap-1">
              ...
              <button
                ...
                onClick={() => {
                  showModal(
                    <UpdateStationForm station={e} onUpdate={onUpdate} />,
                    "Update Station",
                    "Close"
                  );
                }}
