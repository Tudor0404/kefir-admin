  ...
  return (
    ...
    >
      <div className="overflow-x-auto w-full ">
        <table
          className={`table ${props.compact && "table-xs"} ${props.style}`}
        >
          <thead>
            <tr>
              {props.numbered && <th className="select-none">#</th>}
              {props.headers.map((e, i) => (
                <th
                  key={i}
                  className="select-none cursor-pointer"
                  onClick={() => {
                    if (props.sortable && !props.sortable[i]) return;
      ...
      <div className="w-full flex flex-row justify-between items-center">
        <div className="join">
          ...
          <button className="join-item">Page {currentPage}</button>
          <button
            className="join-item"
            type="button"
            onClick={() =>
        ...
        <div className="flex flex-row gap-1">
          ...
          {props.refresh && (
            <button
              className="btn-base border-base-100 "
